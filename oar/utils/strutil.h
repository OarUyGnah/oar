...
template <typename T = const char *> size_t findFirst(std::string str, T c) {
  return str.find(c);
}
template <typename T = const char *> size_t findLast(std::string str, T c) {
  return str.rfind(c);
}
...
template <> inline size_t rfindNth<char>(std::string &str, char c, size_t nth) {
  if (nth <= 0)
    return str.size();
  int pos = str.size() - 1;
  int idx = pos;
